...
//!
//! The PreToolUse / PermissionRequest hooks fire synchronously from the
//! coding agent, so the interceptor's parent is the agent process itself.
//! Capturing this PID lets downstream consumers (e.g. a vanilla Falco
//! running side-by-side with the syscall driver) correlate a hook event
//! with the syscall events emitted by the same agent instance — the
//! agent's children inherit it as `proc.apid[]`.
